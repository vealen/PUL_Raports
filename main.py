...
class mainWindow(QMainWindow, Ui_MainWindow):
    ...
    def final(self):
        if not self.db:
            ...
            if raport_name_user and adr_for[1] and not raport_name_user.isspace():

                # adr_for = self.construct_adr_for()
                count, wybrane_raporty = self.check_chosen_raports()  # ilosc wybranych raportow
                    ...
                    for raport_category in RAPORTS:
                        for raport_name in RAPORTS[raport_category]:
                            ...
                            if RAPORTS[raport_category][raport_name][1]:  # #teoretycznie moznabyloby sprawdzic check
                                ...
                                #     if col in ['Pow','SPECIAL_AREA', 'SumaOfSUB', 'Pow wskazania', 'SUB_AREA','pow','PROT_SITE_AREA',]:
                                #         print('jestem w round 2')
                                #         data[col] = data[col].round(2)
                                #     if col in ['AROD_LAND_USE_AREA','LAND_USE_AREA']:
                                #         print('jestem w round 4')
                                #         data['AROD_LAND_USE_AREA'] = data['AROD_LAND_USE_AREA'].round(4)
